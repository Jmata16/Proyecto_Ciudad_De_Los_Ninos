...
namespace Proyecto_Ciudad_De_Los_Ninos.Controllers
{
    ...
    public class TiendaController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> OrdenConfirmada()
        {
            var userIdClaim = User.Claims.FirstOrDefault(c => c.Type == "UserId")?.Value;
            if (userIdClaim == null || !int.TryParse(userIdClaim, out int userId))
            {
            ...
            // Registrar la compra en la tabla RegistroCompra
        ...
        public async Task<IActionResult> EliminarDelCarrito(int id)
        {
            ...
            var item = await _context.Tickete
                .Include(t => t.inventario_Higiene_Personal) // Incluye el producto para obtener su ID
                .FirstOrDefaultAsync(t => t.id_usuario == userId && t.Id == id);
            ...
            var producto = await _context.Inventario_Higiene_Personal
                .FirstOrDefaultAsync(p => p.Id == item.id_inventario_higiene_personal);
            ...
            producto.cantidad_disponible += item.tickete;
            ...
            {
                return BadRequest("Error al eliminar el producto del carrito. Inténtelo de nuevo más tarde.");
            }
        ...
        private byte[] GeneratePDF(List<Tickete> productosCarrito, string clienteNombre)
        {
            using (var stream = new MemoryStream())
            {
                ...
                gfx.DrawImage(logo, margin, yPos, 125, 100);  // Ajustar tamaño y posición según necesites
